...
def train(window_size=20, starting_balance=100000, resume_epoch=0, max_epoch=1000):
    ...
    for n_epi in pbar:
        if start_epoch == 0:
            epsilon = max(EPSILON_SIZE, 1.0 - 1.0 * n_epi / epochs)
        else:
            epsilon = EPSILON_SIZE #EPSILON_SIZE
...
def test(window_size=20, starting_balance=100000, model_epi='final'):
    ...
    while not done:
        ...
        close_history.append(env.cur_close_price)
        share_history.append(env.shares)
        date_history.append(test_data.index[env.cur_step])
        # q_history.append(q_out.detach().numpy())
